...
// Differents (A-B) unsorted array O(n2)
void differentsUnsortedArray(struct array arA, struct array arB, struct array *arC)
{
    int i, j, k, flag;
    k = 0;
    for (i = 0; i < arA.length; i++)
...
// Differents (A-B) unsorted array O(n2)
void differentsSortedArray(struct array arA, struct array arB, struct array *arC)
{
    int i, j, k;
    i = j = k = 0;
    while (i < arA.length && j < arB.length)
...
int main()
{
    ...
    display(arrA);
    display(arrB);
    differentsSortedArray(arrA, arrB, &arrC);
    display(arrC);
    return 0;
}